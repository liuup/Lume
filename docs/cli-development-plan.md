...
7. **发布流程中的 CLI 冒烟验证**
   - 当前 GitHub Actions release workflow 会在 Windows 绿色版、Windows 安装版和 Linux `.deb` 安装后执行 CLI 冒烟测试。
   - 这意味着 CLI 已经不只是开发辅助工具，而是当前发布质量门禁的一部分。
...
- 对外文档、README 与发布说明应优先使用子命令风格示例：`list` / `search` / `import` / `export` / `open`。
- `--list-papers` 这类旧参数只保留为兼容层和现有 workflow 冒烟测试说明，不再作为主文档入口。
