...
| **AI 基础设施** | 部分完成 | 已支持用户在设置中配置 OpenAI-compatible API Key / Completion URL / Model、summary/translation system prompt、翻译引擎切换（Google / Bing / LLM），并打通本地缓存；尚无 provider 抽象、统一任务调度体系，以及真正结构化的 AI 结果模型 |
...
┌──────────────────────────▼────────────────────────────┐
│                    AI 推理层                            │
│  本地路径：Ollama / 本地模型（目标设计，当前未接入）      │
│  云端路径：OpenAI-compatible Completion API（当前已接入）│
│  向量层：本地 Hnswlib 或 SQLite-vec（目标设计）          │
└──────────────────────────┬────────────────────────────┘
                           │
┌──────────────────────────▼────────────────────────────┐
│                    SQLite 数据库                        │
...
CREATE TABLE ai_briefs (
  ...
  model_provider TEXT NOT NULL,   -- 目标设计：'openai_compatible', 'ollama', 'local'
...
### 6.5 Prompt 管理（目标设计，非当前实现）

当前实现里，summary / translation system prompt 主要来自设置项与数据库持久化，还没有独立的 prompt 资源目录。

如果后续要扩展成可维护的 AI 子系统，建议演进为：
...
> 优先级顺序已调整：F3 已有可用雏形，F1/F2 也已有基础能力。下一步重点不应再是“先做最小 OpenAI 调用”，而应转向结构化结果、解释链路和更清晰的 AI 架构边界。

**Week 1**
1. 把当前散落在 `library_commands.rs` 中的 AI 调用逻辑抽出为独立模块，至少拆出 provider 调用与 prompt 组装层
2. 为 `summarize_document` 定义结构化输出 schema，逐步从自由文本总结过渡到可回链的 JSON brief
3. 在 `db.rs` 中补充独立的 AI brief / task 表，而不是只依赖 summary cache
4. 为 `translate_selection` 之外的解释类任务定义统一接口，例如 `explain_selection(text, context, task_type)`
5. 在前端为 AI 结果明确区分 `loading / cached / fresh / error` 状态，而不是统一显示为普通内容块

**Week 2**
6. 为 AI 总结中的结论补页码来源，打通点击结论跳页
7. 扩展选中文本浮层，从“翻译弹层”演进到“翻译 / 解释 / 追问”的统一入口
8. 回补 F3 的高亮原文绑定能力，使批注整理不再只依赖文字批注
9. 明确未来 provider 边界：当前文档统一按“OpenAI-compatible 已接入，其它 provider 为目标设计”表述
10. 统一缓存失效策略，让模型、语言、prompt 变化都能可靠触发重新生成
