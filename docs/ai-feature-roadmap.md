...
- 新增：`src-tauri/src/library_commands.rs` 中 `digest_annotations(item_id, model_config)` 命令
- 新增：Notes Tab 内的 AI 草稿预览组件 `AnnotationDigestPreview.tsx`
...
- [ ] **F1** 论文速览卡
  - [ ] 后端：`extract_text_for_ai` + `generate_paper_brief` 命令
  - [ ] 数据库：`ai_briefs` + `ai_tasks` 表
  - [ ] 前端：`AIBriefTab.tsx` + 结果渲染 + 跳页联动
  - [ ] 设置：AI 提供方配置入口
- [ ] **F2** 选中文本解释
  - [ ] 前端：`TextLayer.tsx` 选中检测 + 浮动菜单
  - [ ] 前端：`AIExplanationDrawer.tsx` 右侧抽屉
  - [ ] 后端：`explain_selection` 命令
- [ ] **F3** 批注整理助手
  - [ ] 后端：`digest_annotations` 命令
  - [ ] 前端：Notes Tab 内 AI 草稿预览 + 逐条采纳 UI
...
> 优先级顺序：先让 AI 速览卡（F1）跑通End-to-End，建立 AI 调用的基础管道，其他功能可复用。
